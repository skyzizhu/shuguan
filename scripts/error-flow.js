...
const retryReview = document.getElementById("retryReview");
...
firstScreen.addEventListener("wheel", (event) => {
  ...
  if (currentInputMode === "text" && event.target === contentInput && contentInput.scrollHeight > contentInput.clientHeight + 1) {
    return;
  }
...
firstScreen.addEventListener("touchend", (event) => {
  if (!hasResultScreen || !touchStartY) return;
