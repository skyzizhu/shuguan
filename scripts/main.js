...
const initialFollowupMessage = "首轮审核已完成，如需进一步审查、查看解释说明或优化文案内容，可在这里继续与我交流。";
...
function appendPendingUserMessage(text) {
  ...
  article.innerHTML = `
    ...
    <div class="thread-content">
      <div class="thread-message-state is-pending" aria-label="提交中" title="提交中">
        ${getThreadStateIconMarkup("pending")}
      </div>
      <div class="thread-bubble">
        <p>${escapeHtml(text)}</p>
      </div>
...
function getThreadStateIconMarkup(state) {
  if (state === "failed") {
    return `
      <svg class="thread-message-state-icon" viewBox="0 0 24 24" aria-hidden="true">
  ...
  return `<span class="thread-message-state-spinner" aria-hidden="true"></span>`;
}
...
function updateUserMessageState(article, state) {
  ...
  if (state === "success") {
    ...
    stateNode.setAttribute("aria-label", "提交成功");
    stateNode.setAttribute("title", "提交成功");
    stateNode.innerHTML = getThreadStateIconMarkup("pending");
    return;
  }

  stateNode.hidden = false;
  stateNode.className = `thread-message-state is-${state}`;
  stateNode.innerHTML = getThreadStateIconMarkup(state);
  if (state === "failed") {
    stateNode.setAttribute("aria-label", "提交失败，点击重试");
    stateNode.setAttribute("title", "提交失败，点击重试");
    return;
  }

  stateNode.setAttribute("aria-label", "提交中");
  stateNode.setAttribute("title", "提交中");
}
...
function buildAssistantReply(prompt, dimension, mode) {
  const meta = followupDimensionMeta[dimension] || followupDimensionMeta.quality;
  if (mode === "explain") {
    return `复审判断：我会重新核对首轮结论的依据，重点看触发句、上下文语境和是否存在过度放大风险的问题。\n纠偏说明：如果发现首轮结论解释不充分，复审会补充更明确的触发原因和判断范围。\n下一步：如果你认可这轮复审方向，可以在顶部提示区点击“重新审核”更新结果。`;
  }

  if (mode === "strategy") {
    return `复审判断：我会结合目标发布场景重新评估这篇内容，而不是只沿用首轮通用结论。\n场景说明：不同平台对标题、首屏表达、事实依据和舆情放大点的敏感程度不同，复审会按对应场景重新聚焦。\n下一步：如果你认可这轮复审方向，可以在顶部提示区点击“重新审核”更新结果。`;
  }

  return `复审判断：我会围绕 ${meta.label} 重新核查首轮结论是否存在判断过宽、证据不足或忽略上下文的问题。\n补充说明：${meta.response}\n下一步：如果你认可这轮复审方向，可以在顶部提示区点击“重新审核”更新结果。`;
}
